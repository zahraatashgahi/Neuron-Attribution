...
def add_sparse_args(parser):
    ...
    parser.add_argument('--fix', action='store_true', help='Fix topology during training. Default: False.')
...
class Masking(object):
    def __init__(self, optimizer, prune_rate=0.3, growth_death_ratio=1.0,
                  prune_rate_decay=None, death_mode='magnitude',
                    growth_mode='momentum', redistribution_mode='momentum',
                      threshold=0.001, args=None, train_loader=None,
                        device=None, dense_params = 0):
        growth_modes = ['random', 'momentum', 'momentum_neuron', 'gradient', 'attr']
        ...
        self.device = args.device # torch.device("cuda")
        ...
        if self.args.fix:
            print('\nFixing topology during training.\n')
            self.prune_every_k_steps = None
        else:
            print('\nUpdating topology every {0} steps.\n'.format(self.args.update_frequency))
    ...
    def init(self, mode='ER', density=0.05, erk_power_scale=1.0, grad_dict=None):
        ...
        print('Total parameters under sparsity level of {0:.3f}: {1}'.format(density, sparse_size / total_size))
        ...
        if self.prune_every_k_steps is not None:
                if self.args.method == 'GraNet':
                    ...
                    if self.steps % self.prune_every_k_steps == 0:
                        ...
                        self.print_nonzero_counts()
                elif self.args.method == 'DST_random':
                    if self.steps % self.prune_every_k_steps == 0:
                        self.truncate_weights_random()
                        self.print_nonzero_counts()
    ...
    def truncate_weights_random(self, step=None):
    ...
    def gradient_growth(self, name, new_mask, weight):
        ...
        idx_not_to_select = (new_mask==1)
        #print((new_mask==0).sum().item())
        grad = grad*(new_mask==0).float()
        grad = torch.abs(grad)
        grad[idx_not_to_select] = -1
        #print((grad==0).sum().item())      
        #print(" grad --> ", list(grad.size()))
        y, idx = torch.sort(grad.flatten(), descending=True)
    ...
    def gather_statistics(self):
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                ...
                #print(name, self.name2nonzeros[name])
